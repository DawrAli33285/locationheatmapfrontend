...
export default function PatientHeatMap() {
  ...
  const processFile = useCallback(async (file) => {
    ...
    try {
      ...
      // 2. Geocode each address returned from backend
      const geocoded = {};
      for (const { address, cityStateZip, count } of addresses) {
        ...
        if (!coords && zip && ZIP_COORDS[zip]) coords = ZIP_COORDS[zip];
        if (coords) {
          geocoded[`${address}||${cityStateZip}`] = { coords, count, label: address || cityStateZip };
        }
      }
      setZipData(geocoded);
